...
import type { RefreshTokenPayload } from "@/shared/auth/jwt";
import {
    refreshTokenOptions,
    signAccessToken,
    signRefreshToken,
...
class AuthService {
    ...
    public async refreshToken(refreshToken: string) {
        ...
        // Compare hash
        const isValid = await compareValue(refreshToken, storedToken.tokenHash);
        ...
        if (storedToken.expiresAt <= new Date()) {
            await this.userService.revokeRefreshToken(user.id, payload.tokenId);
        ...
        const expiresAt = calculateExpirationDate(config.jwt.refreshExpiresIn);
    ...
    async logout(refreshToken: string) {
        ...
        await this.userService.revokeRefreshToken(payload.userId, payload.tokenId);
